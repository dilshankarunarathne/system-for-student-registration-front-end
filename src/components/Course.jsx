...
const Course = () => {
    ...
    const [course, setCourse] = useState({ data: { code: '', academic_year: '', enrolled_students_sid: [], credits: '' } });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const lecturerResponse = await fetch('http://127.0.0.1:8000/api/lecturer', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${OAUTH_TOKEN}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ lid: data.data.lecturer_id }),
            });
